...
fm_ef = 0.73
ksus = 10
outdir_st = file.path(outdir, sprintf('FRED_%d_school_psa_asymp_%.2f_fm_%.2f_ksus_%.2f_mov_out', ss, asymp_inf,fm_ef, ksus))
...
legend("top", legend = grades_scenarios, col = col_palette[1:4], lwd = c(2,2,2,2), lty = c(1,1,1,1), cex = 0.8, horiz = T)
...
for(cc in 1:length(capacity_list)){
    ...
    for(ii in 1:length(grades_scenarios)){
        ...
        if(ii == 1){
            plot(fred_scenario$Date, fred_scenario$CF_median, type = "l", col = col_palette[ii], xlab = "", ylab  = "", lwd = 1, ylim = c(0,200),xlim = c(as.Date("2020-01-01"), max(tmp_fred$Date)), xaxt = 'n', xaxs = 'i', yaxs = 'i', las = 2, yaxt = 'n')
            if(cc == 1){
