...
pcr_sens = 0.85
ag_sens = 0.75
fit_ratio_df = read_csv('../output/calibrated_ratio_model_test.csv')
fit_ratio_ag = fit_ratio_df$Ratio[fit_ratio_df$Test == "AG"]
fit_ratio_pcr = fit_ratio_df$Ratio[fit_ratio_df$Test == "PCR"]
##===============================#
## PLOT CF grades(cap) --------
##===============================#
...
jpeg('../figures/manuscript_figure_bogota_school_reopening_grades_testpos.jpeg', width=6.5, height=2.2, units="in", res = 300)
layout(rbind(rep(1,5),matrix(2:6,nrow = 1, byrow = T)),
...
for(cc in 1:length(capacity_list)){
    ...
    for(ii in 1:length(grades_scenarios)){
        ...
        if(ii == 1){
            plot(fred_scenario$Date, fred_scenario$ProbPosPCR_median, type = "l", col = col_palette[ii], xlab = "", ylab  = "", lwd = 1, ylim = c(0,0.5),xlim = c(as.Date("2020-01-01"), max(tmp_fred$Date)), xaxt = 'n', xaxs = 'i', yaxs = 'i', las = 2, yaxt = 'n')
            if(cc == 1){
                mtext("Proportion PCR positive", side = 2, line = 2.5, cex = 0.8)
                axis(2,las = 2)                
            }
            grid(ny = 5, nx = NA)
        }else{
            lines(fred_scenario$Date, fred_scenario$ProbPosPCR_median, lwd = 1, 
                  col = col_palette[ii]) 
        }
    }
