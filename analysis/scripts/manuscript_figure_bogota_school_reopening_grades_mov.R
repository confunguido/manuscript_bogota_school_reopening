...
jpeg('../figures/manuscript_figure_bogota_school_reopening_grades_mov.jpeg', width=6.5, height=4.2, units="in", res = 300)
...
tmp_fred_int_mov = tmp_fred_mov %>% filter(intervention_id %in% sprintf('%s-cap%.2f',grades_capacity_scenarios[,1], grades_capacity_scenarios[,2])) %>% dplyr::select(intervention_id, CF_median) %>%
    separate(intervention_id, c('Grade', 'Capacity'), sep = "-") %>%
    spread(key = Capacity, value = CF_median) %>%
    column_to_rownames("Grade")

tmp_fred_int_mov = tmp_fred_int_mov[grades_scenarios,]
...
## Susceptibility high
## Filter grades
tmp_fred_int = tmp_fred_sus %>% filter(intervention_id %in% sprintf('%s-cap%.2f',grades_capacity_scenarios[,1], grades_capacity_scenarios[,2])) %>% dplyr::select(intervention_id, CF_median) %>%
    separate(intervention_id, c('Grade', 'Capacity'), sep = "-") %>%
    spread(key = Capacity, value = CF_median) %>%
    column_to_rownames("Grade")
...
tmp_fred_int_mov = tmp_fred_sus_mov %>% filter(intervention_id %in% sprintf('%s-cap%.2f',grades_capacity_scenarios[,1], grades_capacity_scenarios[,2])) %>% dplyr::select(intervention_id, CF_median) %>%
    separate(intervention_id, c('Grade', 'Capacity'), sep = "-") %>%
    spread(key = Capacity, value = CF_median) %>%
    column_to_rownames("Grade")
...
tmp_fred_low = tmp_fred_sus %>% filter(intervention_id %in% sprintf('%s-cap%.2f',grades_capacity_scenarios[,1], grades_capacity_scenarios[,2])) %>% dplyr::select(intervention_id, CF_low) %>%
    separate(intervention_id, c('Grade', 'Capacity'), sep = "-") %>%
    spread(key = Capacity, value = CF_low) %>%
    column_to_rownames("Grade")
...
tmp_fred_high = tmp_fred_sus %>% filter(intervention_id %in% sprintf('%s-cap%.2f',grades_capacity_scenarios[,1], grades_capacity_scenarios[,2])) %>% dplyr::select(intervention_id, CF_high) %>%
    separate(intervention_id, c('Grade', 'Capacity'), sep = "-") %>%
    spread(key = Capacity, value = CF_high) %>%
    column_to_rownames("Grade")
...
axis(side = 2, las = 1)
axis(side = 1, labels = grades_labels, at = tmp_bar[3,],cex.axis = 0.4, tick = FALSE, line = -0.5)
abline(h = tmp_fred_sus$CF_median[tmp_fred_sus$intervention_id == "Closed"], lwd = 1, lty = 3)
abline(h = tmp_fred_sus$CF_low[tmp_fred_sus$intervention_id == "Closed"], lwd = 0.5, lty = 3)
abline(h = tmp_fred_sus$CF_high[tmp_fred_sus$intervention_id == "Closed"], lwd = 0.5, lty = 3)

abline(h = tmp_fred_sus$CF_median[tmp_fred_sus$intervention_id == "OpenUniversities"], lwd = 1, lty = 3, col = "red")
abline(h = tmp_fred_sus$CF_low[tmp_fred_sus$intervention_id == "OpenUniversities"], lwd = 0.5, lty = 3, col = "red")
abline(h = tmp_fred_sus$CF_high[tmp_fred_sus$intervention_id == "OpenUniversities"], lwd = 0.5, lty = 3, col = "red")

text(x = max(tmp_bar[,]) + 0.3, y =  tmp_fred_sus$CF_median[tmp_fred_sus$intervention_id == "OpenUniversities"]*1.2, "Fully\nOpened", col = "red", cex = 0.35, adj = 0)
text(x = max(tmp_bar) + 0.5, y =  tmp_fred_sus$CF_median[tmp_fred_sus$intervention_id == "Closed"]*1.2, "Closed", col = "black", cex = 0.35, adj = 0)
...
print('Same susceptibility')
(tmp_fred_sus %>% filter(intervention_id %in% c("Closed", "Open", "OpenUniversities")))

for(aa in c('CF_median', 'CF_low', 'CF_high')){
    tmp_fred_int = tmp_fred_sus %>% filter(intervention_id %in% sprintf('%s-cap%.2f',grades_capacity_scenarios[,1], grades_capacity_scenarios[,2])) %>% dplyr::select(intervention_id, !!aa) %>%
        separate(intervention_id, c('Grade', 'Capacity'), sep = "-") %>%
        spread(key = Capacity, value = !!aa) %>%
        column_to_rownames("Grade")
    print(aa)
