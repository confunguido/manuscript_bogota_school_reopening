...
grades_scenarios = c('prek', 'prekprimary', 'primary', 'secondary')
...
jpeg('../figures/manuscript_figure_bogota_school_reopening_income_timeseries.jpeg', width=6.5, height=3.5, units="in", res = 300)
...
legend("top", legend = income_labels, col = col_palette[1:4], lwd = c(2,2,2,2), lty = c(1,1,1,1), cex = 0.8, horiz = T)

## income-based reopening
for(cc in 1:length(capacity_list)){
    tmp_capacity = capacity_list[cc]
    for(ii in 1:length(income_scenarios)){
        fred_scenario = tmp_fred %>% filter(intervention_id %in% sprintf('%s-cap%.2f',income_scenarios[ii],tmp_capacity))
        if(ii == 1){
    ...
    mtext(sprintf("Capacity: %.0f%%", tmp_capacity * 100), side = 3, cex = 0.8)
}
...
for(cc in 1:length(capacity_list)){
    ...
    for(ii in 1:length(income_scenarios)){
        fred_scenario = tmp_fred %>% filter(intervention_id %in% sprintf('%s-cap%.2f',income_scenarios[ii],tmp_capacity))
        if(ii == 1){
            plot(fred_scenario$Date, fred_scenario$AR_median, type = "l", col = col_palette[ii], xlab = "", ylab  = "", lwd = 1, ylim = c(0,0.6),xlim = c(as.Date("2020-01-01"), max(tmp_fred$Date)), xaxt = 'n', xaxs = 'i', yaxs = 'i', las = 2,yaxt = 'n')
            if(cc == 1){
                mtext("Attack rate", side = 2, line = 2.5, cex = 0.6)
                axis(2, las =2)
            }
