...
jpeg('../figures/manuscript_figure_bogota_school_reopening_counterfactuals.jpeg', width=6.5, height=3.8, units="in", res = 300)
...
for(ii in 1:length(scenarios_list)){
    ...
    tmp_scenario = fred_scenario %>% filter(Date > as.Date('2021-02-05'))
    print(sprintf("Scenario: %s, deaths %.2f", scenarios_list[ii], max(tmp_scenario$CF_median)))
    if(ii == 1){
...
for(ii in 1:length(scenarios_list)){
    ...
    if(ii == 1){
        ...
        axis(2,las = 2)
        date_ticks =  seq(from=min(fred_scenario$Date),to=max(fred_scenario$Date), by = '3 month')
        date_labels = month(date_ticks,label = T)
        axis.Date(1,fred_scenario$Date, at = date_ticks, labels = date_labels, cex.axis = 0.8, las = 2)
        grid(ny = 5, nx = NA)
    }else{
