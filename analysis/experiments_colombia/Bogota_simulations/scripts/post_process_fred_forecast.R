...
process_census_tract <- function(fred_key, fred_n,loc_census_df){
    ...
    ## Process school data here    
    school_df = post_process_schools_df(infections_df, fatalities_df, fred_key, fred_n)
    return(left_join(school_df, census_tract_df, by = c("Day")))
}

post_process_schools_df <- function(infections_df, fatalities_df, fred_key, fred_n){
    ##browser()
    ## infections_df = get_infections_df(fred_key, fred_n)
    ## fatalities_df = get_infections_CF_df(fred_key, fred_n)
    school_infections =  infections_df %>% filter(is_student == 1 | is_teacher == 1)
    fatalities_df = fatalities_df %>% left_join(infections_df[infections_df$host %in% fatalities_df$host,c('host','at', 'is_teacher', 'is_student')], by = c("host" = "host"))
    
    ## 1. Calculate students + teachers Attack Rate
    ## 2. Calculate symptomatic attack rate in schools
    ## 3. Calculate fatalities of infections in schools
    ## data frame: day, students_infections, students_symptomatics, students_deaths, teachers_infections, teachers_symptomatics, teachers_deaths
    
    ## 4. Find secondary infections per student/teacher in school and outside of school
    ## 5. Find number of secondary infections that result in symptoms
    ## 6. Find number of secondary infections that result in death

    ## These are fatalities/infections for people who are students
    school_fatalities = fatalities_df %>%
        filter(is_student == 1 | is_teacher == 1) %>%
        group_by(day, is_teacher) %>%
        summarize(deaths = n()) %>% ungroup() %>%
        spread(key = is_teacher, value = deaths) %>%
        bind_rows(tibble(day = -1, '0' = 0, '1' = 0)) %>%
        rename(student_deaths = '0', teacher_deaths = '1') %>%
        replace_na(list(teacher_deaths = 0, student_deaths = 0)) %>%
        filter(day >= 0)

    school_infections = infections_df %>%
        filter(is_student == 1 | is_teacher == 1) %>%
        group_by(day, is_teacher) %>%
        summarize(infections = n()) %>% ungroup() %>%
        spread(key = is_teacher, value = infections) %>%
        bind_rows(tibble(day = -1, '0' = 0, '1' = 0)) %>%
        rename(student_infections = '0', teacher_infections = '1') %>%
        replace_na(list(teacher_infections = 0, student_infections = 0)) %>%
        filter(day >= 0)
    
    school_symptoms = infections_df %>%
        mutate(symp1 = as.numeric(symp1)) %>%
        filter(is_student == 1 | is_teacher == 1) %>%
        filter(symp1 >=0 ) %>%
        group_by(symp1, is_teacher) %>%
        summarize(symptoms = n()) %>% ungroup() %>%
        spread(key = is_teacher, value = symptoms) %>%
        rename(day = symp1) %>%
        bind_rows(tibble(day = -1, '0' = 0, '1' = 0)) %>%
        rename(student_symptoms = '0', teacher_symptoms = '1') %>%
        replace_na(list(teacher_symptoms = 0, student_symptoms = 0)) %>%
        filter(day >= 0)   
    ...
    ## Calculate household infections from students + teachers
    school_second_df = as_tibble(infections_df) %>%
        filter(is_student == 1 | is_teacher == 1) %>%
        dplyr::select(host) %>%
        left_join(dplyr::filter(infections_df, is_student == 0 & is_teacher == 0 & at == 'H'),
                  by = c("host" = "infector")) %>%
        rename(infector = host) %>%
        rename(host = host.y) %>%
        drop_na()


    second_infections_df = school_second_df  %>% group_by(day) %>%
        summarize(second_infections = n()) %>% ungroup()

    second_symptoms_df = school_second_df  %>%
        mutate(symp1 = as.numeric(symp1)) %>%
        filter(symp1 >= 0) %>%
        group_by(symp1) %>%
        summarize(second_symptoms = n()) %>% ungroup() %>%
        rename(day = symp1)


    second_fatalities_df = fatalities_df %>%
        filter(host %in% school_second_df$host) %>%
        group_by(day) %>%
        summarize(second_fatalities = n()) %>%
        ungroup()
    
    school_df = left_join(job_df, school_infections, by = c("Day" = "day")) %>%
        left_join(school_symptoms, by = c("Day" = "day")) %>%
        left_join(school_fatalities, by = c("Day" = "day")) %>%
        left_join(second_infections_df, by = c("Day" = "day")) %>%
        left_join(second_symptoms_df, by = c("Day" = "day")) %>%
        left_join(second_fatalities_df, by = c("Day" = "day"))
...
## First, read available variables
conn_vars = file(vars_file, "r")
var_lines = readLines(conn_vars)
close(conn_vars)

cols_output = cols_output[cols_output %in% var_lines]
