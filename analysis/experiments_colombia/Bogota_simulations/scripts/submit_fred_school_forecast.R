...
output.dir = file.path(scratch_dir, sprintf('FRED_%.0f_school_forecast_asymp_%.2f_fm_%.2f_ksus_%.2f_mov',state_code, asymp_infectivity_in, face_mask_transmission_efficacy_in, kids_susceptibility_age_in))
...
## School closure: Open in january 15
...
for(nn in 1:reps){
    ...
    if(extra_rows > 0){
        ...
        week_imp$day = seq(from=max(tmp_df$day) + 1, by = 7, length.out = nrow(week_imp)) # Control here if weekly or daily
...
localidad_list = 1:19
school_capacity_list = c(0.5,0.75,0.9,1.0)
school_schedule_localidad_file = file.path(output.dir, sprintf('school_schedule_localidad_%d_%d.txt',state_code, localidad_list))

school_schedule_income1_file = file.path(output.dir, sprintf('school_schedule_income1_%d.txt',state_code))
school_schedule_income2_file = file.path(output.dir, sprintf('school_schedule_income2_%d.txt',state_code))
school_schedule_income3_file = file.path(output.dir, sprintf('school_schedule_income3_%d.txt',state_code))
school_schedule_income4_file = file.path(output.dir, sprintf('school_schedule_income4_%d.txt',state_code))

school_schedule_prek_file = file.path(output.dir, sprintf('school_schedule_prek_%d.txt',state_code))
school_schedule_prekprimary_file = file.path(output.dir, sprintf('school_schedule_prekprimary_%d.txt',state_code))
school_schedule_primary_file = file.path(output.dir, sprintf('school_schedule_primary_%d.txt',state_code))
school_schedule_secondary_file = file.path(output.dir, sprintf('school_schedule_secondary_%d.txt',state_code))
...
## Write files
{
    ...
    vacation_closure = sprintf("%d %d 1 20 0", as.integer(as.Date('2020-12-15') - as.Date(start_date)),  as.integer(as.Date(school_vacation_end) - as.Date(start_date)))
    ...
    school_income1_lines = c(current_open_school_lines,
                             ...
                             sprintf("%d %d 1 17 %.2f 1",  school_baseline_reopen_date, numDays, school_reduced_capacity_in),
    ...
    school_income2_lines = c(current_open_school_lines,
                             ...
                             sprintf("%d %d 1 17 %.2f 2",  school_baseline_reopen_date, numDays, school_reduced_capacity_in),
                             sprintf("%d %d 1 20 0.0 3",  school_baseline_reopen_date, numDays),
                             sprintf("%d %d 1 20 0.0 4",  school_baseline_reopen_date, numDays)
                             )
    school_income3_lines = c(current_open_school_lines,
                             ...
                             sprintf("%d %d 1 20 0.0 1",  school_baseline_reopen_date, numDays),
                             sprintf("%d %d 1 20 0.0 2",  school_baseline_reopen_date, numDays),
                             sprintf("%d %d 1 17 %.2f 3",  school_baseline_reopen_date, numDays, school_reduced_capacity_in),
                             sprintf("%d %d 1 20 0.0 4",  school_baseline_reopen_date, numDays)
                             )
    school_income4_lines = c(current_open_school_lines,
                             ...
                             sprintf("%d %d 1 20 0.0 2",  school_baseline_reopen_date, numDays),
                             sprintf("%d %d 1 20 0.0 3",  school_baseline_reopen_date, numDays),
                             sprintf("%d %d 1 17 %.2f 4",  school_baseline_reopen_date, numDays,  school_reduced_capacity_in)
                             )

    ## Grades scenarios
    school_prek_lines = c(current_open_school_lines,
                          sprintf("%d %d 6 20 0",  school_baseline_reopen_date, numDays),
                          sprintf("%d %d 1 5 %.2f",  school_baseline_reopen_date, numDays, school_reduced_capacity_in))
    
    school_prekprimary_lines = c(current_open_school_lines,
                                 sprintf("%d %d 11 20 0",  school_baseline_reopen_date, numDays),
                                 sprintf("%d %d 1 10 %.2f",  school_baseline_reopen_date, numDays, school_reduced_capacity_in))
    
    school_primary_lines = c(current_open_school_lines,
                             sprintf("%d %d 1 5 0",  school_baseline_reopen_date, numDays),
                             sprintf("%d %d 11 20 0",  school_baseline_reopen_date, numDays),
                             sprintf("%d %d 6 10 %.2f",  school_baseline_reopen_date, numDays, school_reduced_capacity_in))
    ...
    school_secondary_lines = c(current_open_school_lines,
                               ...
                               sprintf("%d %d 11 17 %.2f",  school_baseline_reopen_date, numDays, school_reduced_capacity_in))
  
    ## Localidad scenarios
    ## 1. Read localidad vs ESC
    ## 2. Loop through localidad and ESC and write files
    for(ll in 1:length(localidad_list)){
        esc_in_loc = sprintf("11001%s", localidad_esc$SCACODIGO[localidad_esc$Localidad == localidad_list[ll]])
        esc_out_loc = sprintf("11001%s", localidad_esc$SCACODIGO[localidad_esc$Localidad != localidad_list[ll]])
        
        school_localidad_lines = c(
            current_open_school_lines,
            sprintf("%d %d 18 20 0",  school_baseline_reopen_date, numDays),       
            sprintf("%d %d 1 17 %.2f 0 %s",  school_baseline_reopen_date, numDays, school_reduced_capacity_in, esc_in_loc),
            sprintf("%d %d 1 17 0.0 0 %s",  school_baseline_reopen_date, numDays, esc_out_loc))
                               
            ## Write reopening files
        fileConn<-file(school_schedule_localidad_file[ll])
        writeLines(school_localidad_lines, fileConn)
        close(fileConn)
    }
    ...
    ## Write reopening files
    fileConn<-file(school_schedule_income1_file)
    writeLines(school_income1_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_income2_file)
    writeLines(school_income2_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_income3_file)
    writeLines(school_income3_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_income4_file)
    writeLines(school_income4_lines, fileConn)
    close(fileConn)

    
    fileConn<-file(school_schedule_prek_file)
    writeLines(school_prek_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_primary_file)
    writeLines(school_primary_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_prekprimary_file)
    writeLines(school_prekprimary_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_secondary_file)
...
intervention_df_closed = intervention_df %>%    
    mutate(school_closure_duration = school_closure_duration_in,
...
intervention_df_open = intervention_df %>%    
    mutate(school_closure_duration = early_closure_duration,           
           face_mask_timeseries_file = facemask_high_file,           
...
intervention_df_open_nfm = intervention_df %>%    
    mutate(enable_shelter_in_place = 0,
...
## Localidades
for(ff in 1:length(localidad_list)){
    intervention_df_tmp = intervention_df %>%    
        mutate(school_closure_duration = early_closure_duration,               
               face_mask_timeseries_file = facemask_high_file,
               school_global_schedule_file = school_schedule_localidad_file[ff],
               intervention_name = sprintf("Schools_localidad-%d",localidad_list[ff]))
    scalars_intervention = bind_rows(scalars_intervention, intervention_df_tmp)
}
...
submit_jobs(experiment_supername_in = sprintf('FRED_BOG_SCHOOL_FORECAST_asymp_%.2f_FM_%.2f_KSUS_%.2f', asymp_infectivity_in, face_mask_transmission_efficacy_in, kids_susceptibility_age_in),
