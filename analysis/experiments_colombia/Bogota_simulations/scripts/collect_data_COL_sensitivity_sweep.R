    ...
    if(length(args) >= 2){
        ...
        if(length(args) >= 3){
            ...
            if(length(args) >= 4){
                ...
                if(length(args) >= 5){
                    mov_factor_in = as.numeric(args[5])
                }
            }
        }    
    }
...
simdir_name = sprintf('FRED_%d_school_psa_asymp_%.2f_fm_%.2f_ksus_%.2f_mov_%.2f',
                      state_input, asymp_infectivity_in,face_mask_transmission_efficacy_in,
                      kids_susceptibility_in, mov_factor_in)
