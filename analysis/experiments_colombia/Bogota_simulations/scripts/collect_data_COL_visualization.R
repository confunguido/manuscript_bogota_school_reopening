...
process_job_data <- function(job_id, output_vis_dir){
    ...
    ## Visualization stuff-------------
    ##========================================#
    vis_file = file.path(fred_dir, "DATA", "OUT","visualization_output.csv")
    file.copy(vis_file, file.path(output_vis_dir, sprintf("visualization_output_%s",job_id )))
    ...
    cols_output = c(cols_output)        

    ## Replace with NA or 0 the columns that don't exist
    job_df[, cols_output[!(cols_output %in% colnames(job_df))]] = 0
...
simdir_name = sprintf('FRED_%d_school_visualization_asymp_%.2f_fm_%.2f_ksus_%.2f_mov',state_input, asymp_infectivity_in,face_mask_transmission_efficacy_in, kids_susceptibility_in)
simdir = file.path(scratch_dir, simdir_name)

if(!dir.exists(file.path(getwd(), 'output','VISUALIZATION'))){
    dir.create(file.path(getwd(), 'output','VISUALIZATION'), recursive = T)
}

fred_results_dir = file.path(simdir,"FRED_RESULTS")
Sys.setenv(FRED_RESULTS=fred_results_dir)

output_dir = file.path(getwd(), 'output','VISUALIZATION',sprintf("%s_%s", simdir_name, "out"))
...
fredtools::fred_gather_data(params=params_file, outdir=output_dir, outfile=fred_outputs,
                            FUN=check_finished_jobs, FUN2=process_job_data, rm_out = TRUE, appendToFile = FALSE, output_vis_dir = output_dir)    
