...
get_infections_df <- function(fred_key, fred_n){
    ...
    cols_req = c('day', 'host','age','infector','at', 'mixing_group_lbl',
                 'size', 'is_teacher', 'is_student', 'sch_census_tract', 'income', 'classroom_lbl', 'school_lbl', 'host_census_tract', 'home_host_id',
                 'home_inf_id')
    cols_vec = c('inf','symp')
...
post_process_schools <- function(fred_key, fred_n, min_day_in = 377, geo_file=""){
    ...
    infections_df = infections_df %>%
        ...
        dplyr::select(day, host, infector, at,symp1, symp2, is_teacher,is_student,home_host_id, home_inf_id,host_census_tract, mixing_group_lbl,classroom_lbl, school_lbl, size, sch_census_tract, income, children, age) %>%
    ...
    infections_df_out = left_join(infections_df, fatalities_df[,c('host','dead')], by = "host") %>%
        ...
        dplyr::select(day, home_host_id, home_inf_id, at,host,infector, mixing_group_lbl, classroom_lbl, school_lbl, size, income, host_census_tract, sch_census_tract,host_upz, sch_upz, is_teacher, is_student,symp1,symp2, dead, age, children) %>%
