...
## School closure
early_school_vacation_end = as.Date('2020-10-15')
...
## Shelter in place-------------------
##==============================================#
post_lockdown_mobility_in = 0.07
...
## School closure-------------------
##==============================================#
## 6 scenarios of grade reopening:
## 1. All < 6,  2. All < 11, 3. 6-10,  4. 11-17, 5. All <11, 50% 11-17
## 4 scenarios of income reopening:
## 1. Income 1, Income 2, INcome 3, Income 4+
school_schedule_income1_file = file.path(output.dir, sprintf('school_schedule_income1_%d_%d.txt',state_code,1:reps))
school_schedule_income2_file = file.path(output.dir, sprintf('school_schedule_income2_%d_%d.txt',state_code,1:reps))
school_schedule_income3_file = file.path(output.dir, sprintf('school_schedule_income3_%d_%d.txt',state_code,1:reps))
school_schedule_income6_file = file.path(output.dir, sprintf('school_schedule_income6_%d_%d.txt',state_code,1:reps))

school_schedule_prek_file = file.path(output.dir, sprintf('school_schedule_prek_%d_%d.txt',state_code,1:reps))
school_schedule_prekprimary_file = file.path(output.dir, sprintf('school_schedule_prekprimary_%d_%d.txt',state_code,1:reps))
school_schedule_primary_file = file.path(output.dir, sprintf('school_schedule_primary_%d_%d.txt',state_code,1:reps))
school_schedule_secondary_file = file.path(output.dir, sprintf('school_schedule_secondary_%d_%d.txt',state_code,1:reps))
school_schedule_primary50secondary_file = file.path(output.dir, sprintf('school_schedule_primary50secondary_%d_%d.txt',state_code,1:reps))
school_schedule_closed_file = file.path(output.dir, sprintf('school_schedule_closed_%d_%d.txt',state_code,1:reps))
school_schedule_open_file = file.path(output.dir, sprintf('school_schedule_open_%d_%d.txt',state_code,1:reps))
for(nn in 1:reps){
    ...
    vacation_closure = sprintf("%d %d 1 20 0", as.integer(as.Date('2020-12-15') - as.Date(start_date)),  as.integer(as.Date('2021-01-15') - as.Date(start_date)))
    
    school_income1_lines = c(initial_closure,vacation_closure,
                             sprintf("%d %d 18 20 0.0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 17 1.0 1",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 2",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 3",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 4",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 5",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 6",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays)
                             )
    school_income2_lines = c(initial_closure,vacation_closure,
                             sprintf("%d %d 18 20 0.0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 1",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 17 1.0 2",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 3",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 4",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 5",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 6",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays)
                             )
    school_income3_lines = c(initial_closure,vacation_closure,
                             sprintf("%d %d 18 20 0.0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 1",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 2",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 17 1.0 3",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 4",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 5",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 6",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays)
                             )
    school_income6_lines = c(initial_closure,vacation_closure,
                             sprintf("%d %d 18 20 0.0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 1",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 2",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 20 0.0 3",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 17 1.0 4",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 17 1.0 5",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 1 17 1.0 6",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays)
                             )
    
    school_prek_lines = c(initial_closure,vacation_closure,
                          sprintf("%d %d 6 20 0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays))
    school_prekprimary_lines = c(initial_closure,vacation_closure,
                                 sprintf("%d %d 11 20 0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays))
    school_primary_lines = c(initial_closure,vacation_closure,
                             sprintf("%d %d 1 5 0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                             sprintf("%d %d 11 20 0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays))
    school_secondary_lines = c(initial_closure,vacation_closure,
                               sprintf("%d %d 1 10 0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                               sprintf("%d %d 18 20 0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays))
    school_primary50secondary_lines = c(initial_closure,vacation_closure,
                                        sprintf("%d %d 11 17 0.5",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                                        sprintf("%d %d 18 20 0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays))
    school_closed_lines = c(initial_closure, vacation_closure,
                            sprintf("%d %d 1 20 0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays))
    
    school_open_lines = c(initial_closure, vacation_closure,
                            sprintf("%d %d 1 17 1.0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                            sprintf("%d %d 18 20 0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays))

    fileConn<-file(school_schedule_income1_file[nn])
    writeLines(school_income1_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_income2_file[nn])
    writeLines(school_income2_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_income3_file[nn])
    writeLines(school_income3_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_income6_file[nn])
    writeLines(school_income6_lines, fileConn)
    close(fileConn)

    
    fileConn<-file(school_schedule_prek_file[nn])
    writeLines(school_prek_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_primary_file[nn])
    writeLines(school_primary_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_prekprimary_file[nn])
    writeLines(school_prekprimary_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_secondary_file[nn])
    writeLines(school_secondary_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_primary50secondary_file[nn])
    writeLines(school_primary50secondary_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_open_file[nn])
    writeLines(school_open_lines, fileConn)
    close(fileConn)

    fileConn<-file(school_schedule_closed_file[nn])
...
intervention_df_open_low_contacts = intervention_df %>%    
    mutate(seed = floor(scalars_sobol_df$seed),
           primary_cases_file = primary_cases_file_in,
           imports_factor = scalars_sobol_df$imports_factor,
           shelter_in_place_file = shelter_time_file,
           ...
           school_global_schedule_file = school_schedule_open_file,
           nursing_home_incidence_importations_factor = scalars_sobol_df$nursing_home_incidence_importations_factor, 
           classroom_contacts = school_contacts_in * 1.1,
           intervention_name = "Schools_OpenLowContacts")

intervention_df_open_low_contacts_nosch = intervention_df %>%    
    mutate(seed = floor(scalars_sobol_df$seed),
           primary_cases_file = primary_cases_file_in,
           imports_factor = scalars_sobol_df$imports_factor,
           shelter_in_place_file = shelter_time_file,
           ...
           nursing_home_incidence_importations_factor = scalars_sobol_df$nursing_home_incidence_importations_factor, 
           classroom_contacts = school_contacts_in * 1.1,
           school_contacts = school_contacts_in * 0.1,
           intervention_name = "Schools_OpenLowContactsNoSchool")

intervention_df_open_med_contacts = intervention_df %>%    
    mutate(seed = floor(scalars_sobol_df$seed),
           primary_cases_file = primary_cases_file_in,
           imports_factor = scalars_sobol_df$imports_factor,
           shelter_in_place_file = shelter_time_file,
           ...
           school_global_schedule_file = school_schedule_open_file,
           nursing_home_incidence_importations_factor = scalars_sobol_df$nursing_home_incidence_importations_factor, 
           classroom_contacts = school_contacts_in * 1.5,
           intervention_name = "Schools_OpenMedContacts")
...
school_schedule_files = data.frame(
    ...
    Schools_primary50secondary = school_schedule_primary50secondary_file,
    ...
    Schools_income6 = school_schedule_income6_file,    
    stringsAsFactors = F)

scalars_intervention = bind_rows(intervention_df_closed, intervention_df_open, intervention_df_open_nfm,
                                 intervention_df_open_low_contacts,  intervention_df_open_low_contacts_nosch,
                                 intervention_df_open_med_contacts, intervention_df_open_all)

for(ff in 1:ncol(school_schedule_files)){
    intervention_df_tmp = intervention_df %>%    
        mutate(seed = floor(scalars_sobol_df$seed),
               ...
               school_reduced_capacity_day = school_reduced_capacity_day_in,
               enable_face_mask_usage = enable_face_mask_usage_in,
               school_global_schedule_file = school_schedule_files[,ff],
               intervention_name = colnames(school_schedule_files)[ff])
