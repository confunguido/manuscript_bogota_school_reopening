...
write_cmd_function <- function(scalars_in, tmpfile){
    ...
    job_cmd_str = sprintf("fred_job -k %s -I %d -p %s -n %.0f; Rscript ./post_process_fred_visualization.R %s %.0f",
                          scalars_in$job_id,
                          scalars_in$run_id,
...
output.dir = file.path(scratch_dir, sprintf('FRED_%.0f_school_visualization_asymp_%.2f_fm_%.2f_ksus_%.2f_mov',state_code, asymp_infectivity_in, face_mask_transmission_efficacy_in, kids_susceptibility_age_in))
...
file.copy('./scripts/post_process_fred_visualization.R',output.dir)
##file.copy('../input_files/params_covid.txt','./input_files/params_covid.txt', overwrite = T) # Don't update, use the one from calibration
file.copy('../input_files/UPZ_Unidad_Catastral.csv', output.dir, overwrite = T)
...
school_reduced_capacity_in = 1.0 ## Just for visualization
...
school_schedule_open_cap_file = file.path(output.dir, sprintf('school_schedule_open_cap_%d.txt',state_code))
...
## Write files
{
    ...
    vacation_closure = sprintf("%d %d 1 20 0", as.integer(as.Date('2020-12-15') - as.Date(start_date)),  as.integer(as.Date('2021-01-25') - as.Date(start_date)))
    ...
    school_open_cap_lines = c(initial_closure, vacation_closure,
                            sprintf("%d %d 1 17 0.35",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
                            sprintf("%d %d 18 20 0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays))

    school_open_universities_lines = c(initial_closure, vacation_closure,
                                       sprintf("%d %d 1 17 1.0",  as.integer(early_school_vacation_end - as.Date(start_date)) + 1, numDays),
    ...
    fileConn<-file(school_schedule_open_cap_file)
...
intervention_df_open_cap = intervention_df %>%    
    mutate(seed = floor(scalars_sobol_df$seed),
           ...
           school_reduced_capacity_day = school_reduced_capacity_day_in,
           enable_face_mask_usage = enable_face_mask_usage_in,
           school_global_schedule_file = school_schedule_open_cap_file,
           nursing_home_incidence_importations_factor = scalars_sobol_df$nursing_home_incidence_importations_factor, 
           intervention_name = "Schools_OpenCap")
...
scalars_intervention = bind_rows(intervention_df_closed, intervention_df_open, intervention_df_open_cap, intervention_df_open_nfm,
                                 intervention_df_open_all)


school_contact_list = c(0.9)
tmp_capacity_ind = 1
...
basename_jobs = sprintf('FRED_%d_school_visualization_asymp',state_code)
...
submit_jobs(experiment_supername_in = sprintf('FRED_BOG_SCHOOL_VISUALIZATION_asymp_%.2f_FM_%.2f_KSUS_%.2f', asymp_infectivity_in, face_mask_transmission_efficacy_in, kids_susceptibility_age_in),
            ...
            FUN = write_cmd_function,cores_in=6,walltime_in = "1:30:00",
            subsys="UGE",
